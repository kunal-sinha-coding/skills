...
Before starting every new experiment, inspect the process table for training, evaluation, accelerator, and experiment-specific processes. Confirm that only the intended current experiment is running, terminate stale processes from previous or duplicate experiments, and verify the process table again before launch. Do not mix logs or GPU workloads from concurrent experiments.
...
During that meta-review, enumerate competing high-level explanations for the underlying failure and propose a small diagnostic or tiny experiment that can distinguish between them. Prefer discriminating evidence when the result would change the next intervention, such as testing whether failures reflect missing capability versus insufficient exploration. This is a general reasoning pattern rather than a prescribed diagnostic; choose the smallest informative test suited to the competing hypotheses and the available evidence. Actually run each selected diagnostic before deciding the next fix, inspect its reported results, and explicitly update the hypotheses and intervention choice based on what the diagnostic disambiguated. Do not leave a proposed diagnostic as an unexecuted idea unless an explicit environment blocker is documented.
